...
`@script`
Now let's say you're in an interview for an analyst position. The interviewer asks you what percentage of the company's contacts made a purchase. Let's first think about the definition of a percentage - we'd need the number of purchases divided by the total number of contacts. That means we need to bring back all rows from the CONTACTS table, since the denominator of the percentage is all contacts. This should clue us in that a LEFT join is necessary.
...
`@part1`
Now, your boss asks "**Can you give me a list of email addresses of all contacts who made a purchase during or after 2015?**"
...
`@script`
Now your boss asks for a list of email addresses for contacts who made a purchase during or after 2015. To answer that we only want to return contacts who are in the sales table. So, we'll flip the order of the tables - we'll choose SALES as the LEFT table this time. We also need to filter by date in the WHERE statement.
...
`@script`
Again, if you were to do a SELECT * to verify, this is what the result would look like. The JOIN is keeping everything from the sales table since it's the left one this time, and only pulling in rows from the contacts table that match based on contact_id. Therefore, only information about contacts who made a purchase is appearing.
...
`@script`
Here's a quick look at the SELECT * for each business question side-by-side for comparison: they look very different don't they? Unlike INNER JOINs, the order of the tables matters for LEFT JOINs, and you may get a different result depending on the data if you flip the tables.
...
`@script`
We've gone through a couple of example business questions that might come in an interview, or a real business setting. Now, try to practice this on your own!

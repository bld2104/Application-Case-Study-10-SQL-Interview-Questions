...
`@script`
The SELECT statement counts the rows that have a sale_id (number of sales) divided by all rows that have a contact_id (total contacts). The full query looks like this:
